...
2) Run the first section. The program will prompt the user to select the .csv file containing the SAL localization data.
3) Prior to running the next section, set the minPTS (minimum number of points) and the Epsilon (radius) in the DBSCAN section of the code. These parameters may need adjusting depending on your data.
   This section may need to be rerun until adequate clusters are achieved. The program then sorts all the clusters into a struct.
